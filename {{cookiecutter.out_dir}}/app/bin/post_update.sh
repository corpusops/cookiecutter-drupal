...
if [ "xok" = "x${USER_CHOICE}" ]; then
    if [ -e "$ROOTPATH/config/partial" ];then
        echo "${YELLOW}  - first we update apply a subprofile 'partial' to handle preconditions like config_ignore configuration file (which prevents unwanted deletions)${NORMAL}"
    ...
    if [ "x${?}" = "x1" ]; then
        bad_exit "Failure in importing the config:partial, please check the previous lines for details."
    fi
